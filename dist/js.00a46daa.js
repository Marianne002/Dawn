...
exports.preloadImages = exports.preloadFonts = exports.lerp = exports.getMousePos = void 0;
...
},{"./gsap-core.js":"../node_modules/gsap/gsap-core.js","./CSSPlugin.js":"../node_modules/gsap/CSSPlugin.js"}],"js/panelItem.js":[function(require,module,exports) {
"use strict";
...
var PanelItem = /*#__PURE__*/function () {
  function PanelItem(el) {
    ...
    this.DOM.titleTexts = _toConsumableArray(this.DOM.title.querySelectorAll('span'));
    this.DOM.subtitle = this.DOM.el.querySelector('.panel__item-subtitle'); // Splitting will run on this

    this.DOM.subtitleText = this.DOM.subtitle.querySelector('h4'); // all subtitle chars 

    this.DOM.subtitleChars = _toConsumableArray(this.DOM.subtitleText.querySelectorAll('.char'));
  ...
  _createClass(PanelItem, [{
    key: "killActiveTimeline",
    value: function killActiveTimeline() {
...
var Panel = /*#__PURE__*/function () {
  function Panel(el) {
    ...
    this.DOM.items = _toConsumableArray(this.DOM.el.querySelectorAll('.panel__item')); // array of Item

    this.items = [];
    this.DOM.items.forEach(function (item) {
      return _this.items.push(new _panelItem.PanelItem(item));
    }); // position the images and the subtitle chars (centered on top of the image)

    this.layout(); // Init events
  ...
  _createClass(Panel, [{
    key: "calcImgPosition",
    value: function calcImgPosition(imgWrap) {
      var imgRect = imgWrap.getBoundingClientRect(); // the second image will be in the center of the panel, while the first and third will be left and right aligned to the panel

      var centeredVal = this.rect.width / 2 - imgRect.width / 2;
      var tx = 0;
    ...
    key: "positionItemChars",
    value: function positionItemChars(item) {
      var imgRect = item.DOM.imgWrap.getBoundingClientRect();

      var _iterator = _createForOfIteratorHelper(item.DOM.subtitleChars.entries()),
          _step;
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          var subtitleCharRect = char.getBoundingClientRect();
          var charPosition = {
    ...
    value: function layout() {
      ...
      this.rect = this.DOM.el.getBoundingClientRect(); // margin between the images

      this.marginImages = parseInt(getComputedStyle(document.body).getPropertyValue('--margin-images'), 10);

      var _iterator2 = _createForOfIteratorHelper(this.items.entries()),
          _step2;
      ...
      try {
        for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
          ...
          var imgPosition = this.calcImgPosition(item.DOM.imgWrap); // set the new position
    ...
    value: function initEvents() {
      ...
      try {
        var _loop = function _loop() {
          ...
          item.DOM.imgWrap.addEventListener('mouseenter', function () {
            return _this2.showItemChars(item);
          });
          item.DOM.imgWrap.addEventListener('mouseleave', function () {
            return _this2.hideItemChars(item);
          });
          item.DOM.imgWrap.addEventListener('click', function () {
            return _this2.openItem(item);
          });
          item.DOM.closeCtrl.addEventListener('click', function () {
            return _this2.closeItem(item);
          });
        };
    ...
    key: "showItemChars",
    value: function showItemChars(item) {
      if (this.isOpen) return;
      item.killActiveTimeline(); // reset
      ...
      .set(item.DOM.subtitleChars, {
        ...
        scale: function scale() {
          return _gsap.gsap.utils.random(1.1, 2.1);
        }
      ...
      .to(item.DOM.subtitleChars, {
        ...
        ease: 'power2.in',
        x: function x(i) {
          return "+=".concat(i % 2 ? 0 : _gsap.gsap.utils.random(-40, 40));
        },
        y: function y(i) {
          return "+=".concat(i % 2 ? _gsap.gsap.utils.random(-40, 40) : 0);
        },
        opacity: 1,
        stagger: 0.01
    ...
    key: "hideItemChars",
    value: function hideItemChars(item) {
      if (this.isOpen) return;
      ...
      item.timelineOut = _gsap.gsap.timeline().addLabel('start', 0).to(item.DOM.subtitleChars, {
        ...
        ease: 'power3.inOut',
        x: function x(i) {
          return "+=".concat(i % 2 ? _gsap.gsap.utils.random(-20, 20) : 0);
        },
        y: function y(i) {
          return "+=".concat(i % 2 ? 0 : _gsap.gsap.utils.random(-20, 20));
        },
        opacity: 0,
        stagger: 0.01
    ...
    value: function openItem(item) {
      if (this.isOpen) return;
      this.isOpen = true;
      this.currentItemIdx = this.items.indexOf(item); //item.killActiveTimeline();
      // all other items (the .imgWrap element)

      var otherImgWrap = this.items.except(item).map(function (item) {
        return item.DOM.imgWrap;
      });

      _gsap.gsap.timeline({
        onStart: function onStart() {
          return item.DOM.el.classList.add('panel__item--open');
        }
    ...
    value: function closeItem(item) {
      ...
      if (!this.isOpen) return;
      this.currentItemIdx = -1; //item.killActiveTimeline();
      ...
      _gsap.gsap.timeline({
        ...
        ease: 'power4.inOut',
        x: function x(_, t) {
          return _this3.calcImgPosition(t).tx;
        }
      }, 'start').to(this.items.except(item).map(function (item) {
        return item.DOM.imgWrap;
      }), {
        duration: 0.7,
        ease: 'power4.inOut',
        ...
        ease: 'power4.inOut',
        x: function x(i) {
          return _gsap.gsap.utils.random(-100, 100);
        },
        y: function y(i) {
          return _gsap.gsap.utils.random(-100, 100);
        },
...
exports.Panel = Panel;
},{"gsap":"../node_modules/gsap/index.js","./panelItem":"js/panelItem.js","splitting/dist/splitting.css":"../node_modules/splitting/dist/splitting.css","splitting/dist/splitting-cells.css":"../node_modules/splitting/dist/splitting-cells.css","splitting":"../node_modules/splitting/dist/splitting.js"}],"js/cursor.js":[function(require,module,exports) {
"use strict";
...
var ep1El = document.querySelector('.start');
var startEl = document.querySelector('#serie__start');
var ep3El = document.querySelector('.end');
var endEl = document.querySelector('#serie__end');
var artistEl = document.querySelector('.artist');
var artistPrezEl = document.querySelector('#artistPrez');
var albumEl = document.querySelector('.album');
var albumPrezEl = document.querySelector('#albumPrez');
var newsEl = document.querySelector('.news');
var newsPrezEl = document.querySelector('#newsPrez');

var panels = _toConsumableArray(document.querySelectorAll('.panel')); //PRELOAD IMG AND FONTS
...
Promise.all([(0, _utils.preloadImages)('.tiles__line-img'), (0, _utils.preloadFonts)('rmd7deq')]).then(function () {
  ...
  ep1El.addEventListener('click', function () {
    return scroll.scrollTo(startEl);
  });
  ep3El.addEventListener('click', function () {
    return scroll.scrollTo(endEl);
  });
  artistEl.addEventListener('click', function () {
    return scroll.scrollTo(artistPrezEl);
  });
  albumEl.addEventListener('click', function () {
    return scroll.scrollTo(albumPrezEl);
  });
  newsEl.addEventListener('click', function () {
    return scroll.scrollTo(newsPrezEl);
  ...
  panels.forEach(function (panel) {
    return new _panel.Panel(panel);
  }); //INITIALIZE CUSTOM CURSOR

  var cursor = new _cursor.default(document.querySelector('.cursor')); // MOUSE EFFECTS

  _toConsumableArray(document.querySelectorAll('a, .panel__item-imgwrap, button')).forEach(function (link) {
...
},{"./utils":"js/utils.js","locomotive-scroll":"../node_modules/locomotive-scroll/dist/locomotive-scroll.esm.js","./panel":"js/panel.js","./cursor":"js/cursor.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
